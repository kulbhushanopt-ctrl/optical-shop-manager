...
- "brand" is the label/brand name text -- this is the only field a row absolutely needs; never guess or invent one that isn't actually legible, and skip a row entirely if you can't make out its brand, but a missing/blank model, category, SKU, HSN, or price is normal and should NOT cause you to skip the row.
- "category" is the closest match among these category codes, based on gender + material + tier text/column on that row (e.g. "GMX", "gents metal exclusive", "ladies sheet", "sunglasses", "non-branded"/"unbranded"/"local" -> the matching code below); use null if no such column/text is present for that row: "LM" (Ladies Metal), "LMX" (Ladies Metal Exclusive), "LS" (Ladies Sheet), "LSX" (Ladies Sheet Exclusive), "LF" (Ladies Frameless), "GM" (Gents Metal), "GMX" (Gents Metal Exclusive), "GS" (Gents Sheet), "GSX" (Gents Sheet Exclusive), "GF" (Gents Frameless), "KB" (Kids Boys), "KBX" (Kids Boys Exclusive), "KG" (Kids Girls), "KGX" (Kids Girls Exclusive), "SG" (Sunglasses), "NB" (Non-Branded).
...
- "purchaseCost" is a plain number (no currency symbol) from an explicit Purchase Price / Cost Price column, if one is present and distinct from the selling price/MRP column, else null.
